import docx
import tkinter as tk
import tkinter.ttk as ttk

def Make_UI_Docx():
    # Create the window
    root = tk.Tk()
    root.title("Illumina Docx Report Maker")
    root.geometry("600x600+600+200")

    # configure the tk
    root.configure(bg='white')
    
    # Input fields Service ID for the report
    Label_ServieID = tk.Label(root, text="Enter the title of your document:", bg="white")
    entry_1 = tk.Entry(root)
    
    # Input fields Service ID for the report
    label_2 = tk.Label(root, text="Enter the body of your document:", bg="white")
    entry_2 = tk.Text(root, height=10, width=50)

    # Define the list of variables Libaries 
    Var_Platform = ['WGS', 'WTS/mRNA Seq', 'WES', 'Methyl', 'de novo', 'Seqeuncing Only', 'Single Cell RNA Sequencing']

    # Library of ComboBox widgets
    Combobox_label = tk.Label(root, text="Select a library", bg="white")
    Combobox_label.grid(row=2, column=0, sticky=tk.E)
    Combobox_Platform = ttk.Combobox(root, height=5, values=Var_Platform)
    Combobox_Platform.set("Library")

    # Place the widgets in the window
    Label_ServieID.grid(row=0, column=0, sticky=tk.E)
    entry_1.grid(row=0, column=1)
    label_2.grid(row=1, column=0, sticky=tk.E)
    entry_2.grid(row=1, column=1)
    Combobox_Platform.grid(row=2, column=1)



    # Create the button
    button = tk.Button(root, text="Create Document", command=None, sticky=None)
    button.grid(row=3, columnspan=2)

    # Run the window
    root.mainloop()